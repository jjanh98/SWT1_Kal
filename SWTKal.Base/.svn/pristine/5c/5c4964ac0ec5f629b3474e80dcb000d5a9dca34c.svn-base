...
//	protected Map<String, Map<String, Vector<Termin>>> besitzerTermine;
	...
	protected SimpleServer()
	{
		...
		try
		{
			// Erstellen eines Benutzers mit dem Namen 'SWT' 
			Person p = new Person("SWT", "SWT", "SWT");
			insert(p, "SWT");
			// Erstellen eines Termins fuer den Benutzer 'SWT'
			insert(new Termin(p, "Vorlesung", "Eine Vorlesung",
					new Datum(new Date()).addDauer(1.5), new Datum(new Date()).addDauer(3.5), getUniqueRunningID()));
		}
		catch (Exception e)
		{
			e.printStackTrace();
...
//		delete(termin, termin.getBesitzer(), besitzerTermine);
	...
	public static int getUniqueRunningID() {
		_runningID++;
		return _runningID - 1;
	}
